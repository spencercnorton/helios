...
This document is for an agent with shell access on a Linux machine that has to install and configure Helios for a person, unattended, and prove that it worked. Every step is a command followed by its expected result, and every path, key, default and message is taken from the source of this release (0.99.4), so a mismatch is a finding, not a typo. It cannot sign the person in to Claude or Codex: those are interactive logins in the CLIs' own credential stores, and the agent hands them back to the person at the points marked below.
...
```bash
git clone --branch v0.99.4 https://github.com/spencercnorton/helios.git ~/helios
PYTHONPATH=~/helios/src python3 -c "import helios; print(helios.__version__)"
```

Expected: the clone prints `Note: switching to '<sha>'.` and a `You are in 'detached HEAD' state` paragraph (normal for a tag checkout), then `0.99.4`. Drop `--branch v0.99.4` to track the newest release instead; the version printed is then whatever `main` carries, and the `0.99.4` strings in this document, 7.3 and 12 are the release it was written for, not a mismatch.
...
Expected: `helios	0.99.4` from `dpkg-query`, `0.99.4` from the other two. `man helios` is also installed by the package.
