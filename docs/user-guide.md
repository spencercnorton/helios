...
| `HELIOS_STATE_DIR` | Root for `logs/`, `work/work.db`, `checkpoints.json`, `checkpoint-trash/`, `conversation-perms.json`, `session-providers.json`, `session-goals.json`, `openrouter-sessions/`, `openrouter-routes.json`, `openrouter-context/` and `AGENTS.md` (default `~/.helios`). `ui-state.json`, `openrouter.key`, `openrouter-models.json`, `model-catalog.json`, `title-cache.json`, `project-names.json`, `session-archive/` and `backups/` always live in `$HOME/.helios` regardless. To relocate everything, run with a different `HOME` |
